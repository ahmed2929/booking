...
passport.use('googleToken', new GooglePlusTokenStrategy({
  ...
  try {
    ...
    //console.log('refreshToken', refreshToken);
      ...
      let existingUser = await TrederUser.findOne({ "google.id": profile.id });
      if (existingUser) {
      ...
      let existingUserLocal = await TrederUser.findOne({ "local.email": profile.emails[0].value })
      ...
      let existingUserfacbook = await TrederUser.findOne({ "facebook.email": profile.emails[0].value })
 ...
 passport.use('facebookToken', new FacebookTokenStrategy({
   ...
   try {
     ...
     console.log('accessToken', accessToken);
       ...
       // We're in the account creation process
      let existingUser = await TrederUser.findOne({ "facebook.id": profile.id });
      if (existingUser) {
        req.user=existingUser
       ...
       let existingUsergoogle = await TrederUser.findOne({ "google.email": profile.emails[0].value })
